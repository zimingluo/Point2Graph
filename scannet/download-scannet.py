...
if __name__ == "__main__": 
    ...
    file_path = './meta_data/scannetv2_val_copy.txt'
    download_scenes = []

    with open(file_path, 'r') as file:
        for line in file:
            download_scenes.append(line.strip())

    # print(len(download_scenes))

    for scan_id in download_scenes:
        args.id = scan_id
        my_main(args)
...
# Script usage:
# - To download the entire ScanNet release (1.3TB):                                             python2 download-scannet.py -o [directory in which to download] 
# - To download a specific scan (e.g., scene0000_00):                                           python2 download-scannet.py -o [directory in which to download] --id scene0000_00
# - To download a specific file type (e.g., *.sens, valid file suffixes listed here):    python2 download-scannet.py -o [directory in which to download] --type .sens
# - To download the ScanNet v1 task data (inc. trained models):                         python2 download-scannet.py -o [directory in which to download] --task_data
...
# - To download the scene dataset in the validation set: python download-scannet.py -o .