...
from data.datasets import PointCloudDataset, my_collate_fn, MPCAT40Dataset

import sys
sys.path.append("./Uni3D/")
from Uni3D.utils import utils
from Uni3D.utils.utils import get_dataset
from Uni3D.utils.tokenizer import SimpleTokenizer
from Uni3D.utils.distributed import is_master, init_distributed_device, world_info_from_env, create_deepspeed_config
from Uni3D.utils.params import parse_args
from Uni3D.utils.logger import setup_logging
...
def main(args):
    ...
    # evaluate model
    if args.evaluate_3d:
        inference(args, model, clip_model)
...
def inference(args, model, clip_model, detected_objects, device, validate_dataset_name='scannet'):
    ...
    # test_dataset = MPCAT40Dataset(detected_objects)
    ...
    with torch.no_grad():
        ...
        # for i, (pc, rgb) in enumerate(test_loader):
            ...
            maxk = 1
            _, pred = logits_per_pc.topk(maxk, 1, True, True)
            pred = pred.t().cpu().numpy()
            label_names = np.vectorize(lambda x: labels[x])(pred)

            # predicted_labels.extend(label_names[0])
            predicted_labels.extend(pred[0])
